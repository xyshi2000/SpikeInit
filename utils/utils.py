...
def tb_record(
    ...
    for layer_name, firing_rate in firing_rate_dict.items():
        tb_writer.add_scalar(f'firing_rate/{layer_name}', firing_rate, epoch)
    for layer_name, gamma in gamma_dict.items():
        tb_writer.add_scalar(f'ASLIF_gamma/{layer_name}', gamma, epoch)
    for layer_name, beta in beta_dict.items():
        tb_writer.add_scalar(f'ASLIF_beta/{layer_name}', beta, epoch)
    for layer_name, weight_std in weight_std_dict.items():
        tb_writer.add_scalar(f'weight_std/{layer_name}', weight_std, epoch)
...
def snn_to_ann_input(x: Union[Tuple[torch.Tensor], torch.Tensor]) -> torch.Tensor:
    ...
    def __init__(self, net: nn.Module):
        ...
        for name, m in net.named_modules():
            ...
            if isinstance(m, nn.Conv2d):
                ...
                self.hooks.append(m.register_forward_hook(self.create_hook(name)))  # type:ignore

    def cal_sop(self, x: Tensor, m: nn.Conv2d):
        with torch.no_grad():
    ...
    def create_hook(self, name):
        def hook(m: nn.Conv2d, x: Tensor, y: Tensor):
            if self.is_enable():
                ...
                self.records.append(self.cal_sop(snn_to_ann_input(x).detach(), m))
...
def count_conv2d(m: nn.Conv2d, x, y: torch.Tensor):
    x = x[0]
